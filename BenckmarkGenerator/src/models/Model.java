			...
			try {
				...
				while ((line = bri.readLine()) != null) {					
					if (line.contains("Generated tuples for 2-wise"))
						ratio = Float.parseFloat(line.substring("Generated tuples for 2-wise with cardinality".length() + 1));
					
					if (line.contains("size dopo controllo copribilita"))
						ratio = Float.parseFloat(line.substring("size dopo controllo copribilita".length() + 1)) / ratio;
				}
