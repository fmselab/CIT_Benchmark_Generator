...
/**
 * Mutation that extends a parameter of the model by removing an element
 */
public class ParameterShrinkerMutation implements MutationOperator<ModelSolution> {
	...
	/**
	 * Builds a new ParameterShrinkerMutation object
	 * 
	 * @param p the probability for applying the mutation
	 */
	public ParameterShrinkerMutation(float p) {
		this.probability = p;
	}
	...
	public Model mutateModel(Model m) {
		...
		if (rng.nextFloat(0, 1) < 0.5 || !m.getParameters().stream().anyMatch(x -> x instanceof Range)) {
			// Shrink an enumerative
			param = mTemp.getRandomParamenterOfClass(new CtWedgeFactoryImpl().createEnumerative());
			if (param == null)
				return m;

			// Get the previously contained elements remove one
			List<Element> elemsList = ((EnumerativeImpl) param).getElements();
			List<Element> newElemsList = new ArrayList<Element>();

			// Remove the element only if the minimum cardinality is not overpassed
			if (elemsList.size() > m.getGeneratorConfiguration().MIN_CARDINALITY) {
			...
			if (ParameterElementsGetterAsStrings.instance.doSwitch(p)
					.size() > m.getGeneratorConfiguration().MIN_CARDINALITY) {
				if (p.getBegin() < m.getGeneratorConfiguration().UPPER_BOUND_INT)
					p.setBegin(p.getBegin() + 1);
				else if (p.getEnd() > m.getGeneratorConfiguration().LOWER_BOUND_INT)
					p.setEnd(p.getEnd() - 1);
			}
		}

		System.out.println("****** Removing a value from the parameter " + param.getName());
		return mTemp;
	}
