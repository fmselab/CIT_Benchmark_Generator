...
public class NSGAIIRatio extends NSGAII<ModelSolution> {
	...
	public NSGAIIRatio(Problem<ModelSolution> problem, int maxEvaluations, int populationSize, int matingPoolSize,
	 ...
	 * found with fitness close to 0, i.e., it is with the desired tuple ratio and
	 * test ratio
	 */
	@Override
	protected boolean isStoppingConditionReached() {
		...
		boolean isOneSolutionWithCorrectRatio = false;

		for (ModelSolution s : this.population) {
			try {
				double tupleRatioDiff = Math
						.abs(s.getModel().getTupleValidityRatio() - s.getModel().getGeneratorConfiguration().RATIO);
				double testRatioDiff = Math
						.abs(s.getModel().getTestValidityRatio() - s.getModel().getGeneratorConfiguration().RATIO_TEST);
				if (tupleRatioDiff <= s.getModel().getGeneratorConfiguration().EPSILON
						&& testRatioDiff <= s.getModel().getGeneratorConfiguration().EPSILON) {
					isOneSolutionWithCorrectRatio = true;
					break;
				}
			} catch (Exception e) {
			}
