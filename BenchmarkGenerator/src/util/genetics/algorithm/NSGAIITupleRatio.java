...
public class NSGAIITupleRatio extends NSGAII<ModelSolution> {
	...
	public NSGAIITupleRatio(Problem<ModelSolution> problem, int maxEvaluations, int populationSize, int matingPoolSize,
	 ...
	 * found with fitness close to 0, i.e., it is with the desired tuple ratio
	 */
	@Override
	protected boolean isStoppingConditionReached() {
		...
		boolean isOneSolutionWithCorrectTupleRatio = false;
		...
		for (ModelSolution s : this.population) {
			try {
				...
				if (ratioDiff <= s.getModel().getGeneratorConfiguration().EPSILON) {
					isOneSolutionWithCorrectTupleRatio = true;
					break;
				}
			} catch (Exception e) {
			}
		}
		boolean condition = stoppingConditionReached || timedOut || (isOneSolutionWithCorrectTupleRatio);
